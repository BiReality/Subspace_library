...
public class BookParser {
	...
	public static List<string> GetPages (string content, int PAGE_WIDTH = 26, int PAGE_HEIGHT = 36) {
		...
		for (int i = 8; i >= 4; i--) {
			string needle = "\n\n\n\n\n\n\n\n";
			content = content.Replace(needle.Substring(0, i), PAGE_BREAK_PLACEHOLDER);
		}
		content = content.Replace("\n\n\n", TRIPLE_NEWLINE_PLACEHOLDER);
		...
		for (int i = 0; i < content.Length; i++) {
			...
			if (cur_char == PAGE_BREAK_PLACEHOLDER) {  // signaled page break
				...
				if (next_word_len > WORD_CUT_LENGTH) {
					...
					if (allowed != 0 && content[i+allowed-1] != ' ') {
						page += "-";
					}
					i += allowed;
				}
